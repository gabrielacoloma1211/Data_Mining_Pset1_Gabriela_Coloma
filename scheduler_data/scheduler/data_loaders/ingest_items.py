...
def get_access_token():
    ...
    #request para sacar access token
...
def _fetch_qb_data(realm_id, access_token, query, base_url, minor_version, start_pos=1, max_results=100):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:  #muchas llamadas
...
def load_data(*args, **kwargs):
    ...
    while current_start < fecha_fin_dt:
        ...
        while True:
            ...
            items = data.get("QueryResponse", {}).get("Item", [])
            if not items:  # si ya no hay más resultados
                break
